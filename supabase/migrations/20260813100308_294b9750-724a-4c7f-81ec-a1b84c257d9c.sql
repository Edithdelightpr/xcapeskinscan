-- 1. Exact session -> assessment relation
ALTER TABLE public.public_analysis_sessions
  ADD COLUMN IF NOT EXISTS assessment_id uuid
    REFERENCES public.client_visit_assessments(id) ON DELETE SET NULL;

CREATE INDEX IF NOT EXISTS public_analysis_sessions_assessment_id_idx
  ON public.public_analysis_sessions (assessment_id);

-- 2. Harden the claim RPC: never fall back to "latest assessment for client".
...
BEGIN
  ...
  -- Recovery requires PROOF: the assessment must belong to this session's
  -- client and carry this session's exact engine payload. Never "latest".
  ...
  -- One assessment per public session, always.
...
-- 3. Immutable public protocol snapshot writer (service-role only).
CREATE OR REPLACE FUNCTION public.public_analysis_store_protocol_snapshot(
...
DECLARE
  v_existing jsonb;
BEGIN
  IF p_assessment_id IS NULL OR p_snapshot IS NULL THEN
    RETURN false;
  END IF;
