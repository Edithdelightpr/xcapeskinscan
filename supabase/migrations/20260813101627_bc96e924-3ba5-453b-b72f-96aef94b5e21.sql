-- 1) One session may never point at an assessment another session owns.
CREATE UNIQUE INDEX IF NOT EXISTS public_analysis_sessions_assessment_id_uniq
  ON public.public_analysis_sessions (assessment_id)
  WHERE assessment_id IS NOT NULL;

-- 2) Exact session identity, including ambiguous legacy rows.
...
BEGIN
  ...
  IF s.client_id IS NOT NULL THEN
    SELECT count(*), min(a.id) INTO v_match_count, v_assessment_id
    FROM public.client_visit_assessments a
...
-- 3) Protocol snapshot storage must report WHY it did not store, so the caller
--    can distinguish "already stored" (fine) from a real failure (retryable).
DROP FUNCTION IF EXISTS public.public_analysis_store_protocol_snapshot(uuid, jsonb);

CREATE OR REPLACE FUNCTION public.public_analysis_store_protocol_snapshot(p_assessment_id uuid, p_snapshot jsonb)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public', 'pg_temp'
...
DECLARE
  v_existing jsonb;
BEGIN
  IF p_assessment_id IS NULL OR p_snapshot IS NULL OR jsonb_typeof(p_snapshot) <> 'object' THEN
    RETURN jsonb_build_object('ok', false, 'stored', false, 'reason', 'invalid_input');
  END IF;

  SELECT skin_analysis -> 'public_protocol_snapshot' INTO v_existing
  FROM public.client_visit_assessments
  WHERE id = p_assessment_id
  FOR UPDATE;

  IF NOT FOUND THEN
    RETURN jsonb_build_object('ok', false, 'stored', false, 'reason', 'assessment_not_found');
  END IF;

  -- Immutable: never overwrite an existing snapshot. Idempotent success.
  IF v_existing IS NOT NULL AND v_existing <> 'null'::jsonb THEN
    RETURN jsonb_build_object('ok', true, 'stored', false, 'reason', 'already_present');
  END IF;

  UPDATE public.client_visit_assessments
  SET skin_analysis = jsonb_set(
        COALESCE(skin_analysis, '{}'::jsonb),
  ...
  RETURN jsonb_build_object('ok', true, 'stored', true, 'reason', 'stored');
END;
$function$;

REVOKE ALL ON FUNCTION public.public_analysis_store_protocol_snapshot(uuid, jsonb) FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.public_analysis_store_protocol_snapshot(uuid, jsonb) TO service_role;