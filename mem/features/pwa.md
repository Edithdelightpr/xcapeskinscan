...
- Install UX: `src/components/pwa/InstallXcape.tsx` (used on the landing page), helpers/tests in `src/lib/pwa/installState.ts`. Android browsers that mint their own legacy-target package (Samsung Internet, in-app webviews) are blocked by Google Play Protect ("Unsafe app blocked"); they resolve to the `android-blocked` affordance and get an "Open in Chrome" intent link instead — only Chrome's Google WebAPK service produces an installable package.
