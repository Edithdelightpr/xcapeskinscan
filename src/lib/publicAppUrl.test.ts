...
describe('canonical origin stability (PWA identity lock)', () => {
  // Installed PWAs are origin-scoped. Renaming the Lovable slug once already
  // orphaned every Android home-screen install; this test exists so the
  // production origin cannot be changed without an explicit, planned migration.
  it('is pinned to the original production origin', () => {
    expect(CANONICAL_PUBLIC_APP_URL).toBe('https://xcapeskinscan.lovable.app');
    expect(CANONICAL_PUBLIC_APP_URL).not.toContain('xcape-skin-scanner');
    expect(new URL(CANONICAL_PUBLIC_APP_URL).protocol).toBe('https:');
    expect(CANONICAL_PUBLIC_APP_URL.endsWith('/')).toBe(false);
  });
});
