...
const XcapeProposalsPanel = ({
  ...
  /**
   * Automatic proposal generation. Once the assessment is saved and the
   * engine scores exist, the published XCAPE criteria are evaluated and any
   * match is recorded as a *proposal* — never applied. The practitioner still
   * accepts, edits or rejects every one. Runs once per assessment.
   */
  const autoGeneratedFor = useRef<string | null>(null);
  useEffect(() => {
    if (!assessmentId || !skin.engine) return;
    if (autoGeneratedFor.current === assessmentId) return;
    if (proposals.length > 0 || previewMatches.length === 0) return;
    if (createMut.isPending) return;
    autoGeneratedFor.current = assessmentId;
    createMut
      .mutateAsync({
        assessmentId,
        clientId,
        matches: previewMatches,
        engineVersion: 'xcape-engine-v1',
      })
      .then(() =>
        toast.info(`${previewMatches.length} XCAPE proposal(s) generated — review each one below`),
      )
      .catch(() => {
        autoGeneratedFor.current = null;
      });
  }, [assessmentId, skin.engine, proposals.length, previewMatches, createMut, clientId]);
