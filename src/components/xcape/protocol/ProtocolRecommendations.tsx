...
import type { BodyDerivation } from '@/lib/xcapeRules/protocol';
                ...
                {p.additions.map((a, i) => (
                  ...
                  >
                    ...
                    <span className="text-[11px]">
                      {a.concern} · health score {a.score}/100 · tier {a.tier_label}
                      {a.derivation && a.derivation.multiplier > 1
                        ? ` · ${a.derivation.base_face_dose_ml} ml face × ${a.derivation.multiplier}`
                        : ''}
                    </span>
                  </li>
                ))}
 ...
 *     their DS solutions and exact ml (body derived from facial findings).
    ...
    >
      <div className="space-y-1">
        ...
        <p className={cn('text-[11.5px]', dark ? 'text-slate-400' : 'text-muted-foreground')}>
          {footnote ??
            'Derived from your four skin-health scores (100 = healthiest) using the confirmed XCAPE customization protocol. The body protocol is derived from the facial findings: the weak-elasticity Body Milk line is prepared at 5× the Face Cream amount.'}
        </p>
      </div>
      ...
      {hasCustomization && (
        <div className="space-y-4">
          ...
          <AreaGroup
            title="Your XCAPE Body Protocol"
            subtitle="derived from the facial findings"
            items={body}
            tone={tone}
          />
        </div>
